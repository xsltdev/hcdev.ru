...
![Список с маркерами outside и inside ::marker, который показывает, что outside (значение по умолчанию) не находится в элементе списка и находится внутри блока содержимого элемента списка.](lists-1.png)
...
![Список с тремя элементами, в котором видно, что каждая из пуль является псевдоэлементом ::marker](lists-2.png)
